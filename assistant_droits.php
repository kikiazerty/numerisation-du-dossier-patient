...
elseif ($mode=='signataire') //on inverse pour les signataires
{
  ...
  $sql_login_delegue=$pdo->prepare('SELECT Login FROM Personnes WHERE GUID = ?');
  $sql_login_delegue->bindValue(1, $guid_friend, PDO::PARAM_STR);
  $sql_login_delegue->execute();
  $ligne_login_delegue=$sql_login_delegue->fetch(PDO::FETCH_ASSOC);
  $sql_login_delegue->closeCursor();
}

$sql_select_droits=$pdo->prepare('SELECT Droits,Login FROM Personnes WHERE GUID= ?');
$sql_select_droits->bindValue(1, $guid_sign, PDO::PARAM_STR);
...
$sql_select_droits_delegues=$pdo->prepare('SELECT FriendUserDroits FROM user_perms WHERE SignataireGUID= ? AND FriendUserGUID= ?');
$sql_select_droits_delegues->bindValue(1, $guid_sign, PDO::PARAM_STR);
$sql_select_droits_delegues->bindValue(2, $guid_friend, PDO::PARAM_STR);
$sql_select_droits_delegues->execute();
$ligne_select_droits_delegues=$sql_select_droits_delegues->fetch(PDO::FETCH_ASSOC);
$sql_select_droits_delegues->closeCursor();
