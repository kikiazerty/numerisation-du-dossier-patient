...
$sql_nom=$pdo->prepare('SELECT * FROM IndexNomPrenom WHERE FchGnrl_IDDos= ?');
$sql_nom->bindValue(1, $patient, PDO::PARAM_STR);
$sql_nom->execute();
...
//$count_nom=mysqli_num_rows($resultat_nom); inutilise
...
$sql_fchpat->bindValue(1, $patient, PDO::PARAM_STR);
$sql_fchpat->execute();
...
$sql_notes=$pdo->prepare('SELECT * FROM fchpat_Note WHERE fchpat_Note_PatPK= ?');
$sql_notes->bindValue(1, $primkey, PDO::PARAM_STR);
$sql_notes->execute();
...
/*
$sql_corresp=$pdo->prepare('SELECT * FROM fchpat_Intervenants INNER JOIN Personnes ON fchpat_Intervenants.fchpat_Intervenants_IntervPK=Personnes.ID_PrimKey WHERE fchpat_Intervenants_PatGUID= ?');
$sql_corresp->bindValue(1, $patient, PDO::PARAM_STR);
$sql_corresp->execute();
$ligne_corresp=$sql_corresp->fetch(PDO::FETCH_ASSOC);
...
echo '
	<tr>
	  <td>
	    '.stripslashes($ligne_fchpat["FchPat_Titre"])." <strong><a href=\"patient.php?GUID=$patient&amp;affichage=rien>\">".stripslashes($nom)." ".stripslashes($ligne_nom["FchGnrl_Prenom"])."</a></strong>, ".$ne." le ".$naissance."<br />";
...
$sql_global=$pdo->prepare('SELECT * FROM RubriquesHead INNER JOIN RubriquesBlobs ON RubriquesBlobs.RbDate_PrimKey=RubriquesHead.RbDate_RefBlobs_PrimKey WHERE RubriquesHead.RbDate_IDDOs= ? ORDER BY RbDate_Date DESC');
...
function verif_login($login_possesseur_doc,$droits_user,$GUID_user,$num_rubrique,$user,$rubrique_perso,$pdo)
{
...
*/ 
    ...
    $sql_utilisateur_autorise->bindValue(2, $ligne_GUID_possesseur_doc['GUID'], PDO::PARAM_STR);
...
/*
 ...
 //     $liste_utilisateur_autorise=mysqli_fetch_array($resultat_utilisateur_autorise);
      if (preg_match("/".$droit."/",$ligne_utilisateur_autorise['FriendUserDroits']))//On verifie qu'il a les droits
	$lectureOK=1;
...
//On verifie que le document trouve est d'un type demande
  foreach ($type_info AS $this_type_info) //On balaye les types recus en URL
  {
    ...
    {
      $lectureOK=verif_login($login_possesseur_doc,$droits_user,$GUID_user,$this_type_info,$user,$rubrique_perso,$pdo); 
  ...
  //le terrain brut avant interpretation
	  ...
	  foreach ($split_terrain as $ligne_brut)
	  {
	    ...
	    if ($atcd==1 and !(preg_match("`\[Ant`i",$ligne_brut)) and !(preg_match("`(\[Obst)`i",$ligne_brut)))
	    {
	      ...
	      $sql_recherche_possesseur->bindValue(1, $login_possesseur_terrain, PDO::PARAM_STR);
...
/*
		...
		$sql_utilisateur_autorise->bindValue(1, $GUID_user, PDO::PARAM_STR);
		$sql_utilisateur_autorise->bindValue(2, $GUID_possesseur_terrain, PDO::PARAM_STR);
		$sql_utilisateur_autorise->execute();
		$ligne_utilisateur_autorise=$sql_utilisateur_autorise->fetch(PDO::FETCH_ASSOC);
		$sql_utilisateur_autorise->closeCursor();
