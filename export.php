...
//ON CHERCHE LES DONNES COMPLEMENTAIRES d'identite
  $sql_chercher_fchpat=$pdo->prepare("SELECT * FROM fchpat WHERE FchPat_GUID_Doss=?");
  $sql_chercher_fchpat->bindValue(1, $patient, PDO::PARAM_STR);
  $sql_chercher_fchpat->execute();
    $ligne_chercher_fchpat=$sql_chercher_fchpat->fetch(PDO::FETCH_ASSOC);
    $sql_chercher_fchpat->closeCursor();
...
*/
  ...
  $sql_notes->bindValue(1, $patient, PDO::PARAM_STR);
  $sql_notes->execute();
...
*/
  ...
  $sql_documents=$pdo->prepare("SELECT * FROM RubriquesBlobs INNER JOIN RubriquesHead ON RubriquesBlobs.RbDate_PrimKey=RubriquesHead.RbDate_RefBlobs_PrimKey WHERE RubriquesBlobs.RbDate_IDDos=?");
  $sql_documents->bindValue(1, $patient, PDO::PARAM_STR);
  $sql_documents->execute();
...
<FchPat_LoginPatient>".$ligne_chercher_fchpat['FchPat_LoginPatient']."</FchPat_LoginPatient>\n
<FchPat_PasswPatient>".$ligne_chercher_fchpat['FchPat_PasswPatient']."</FchPat_PasswPatient>\n
...
while ($ligne_documents=$sql_documents->fetch(PDO::FETCH_ASSOC))
{
