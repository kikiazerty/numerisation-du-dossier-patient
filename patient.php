...
include("templates.php"); //ne pas inclure avant de renseigner nom et prenom //plante php7
...
<div class="taille">
  <div class="groupe">
      <h1>
	<a href="liste.php?critere_recherche=FchGnrl_NomDos&amp;cle=<?php echo stripslashes($nom) ?>&amp;envoyer=Chercher&amp;exact=1" target="_top"><?php echo stripslashes($nom) ?></a>
	<a href="liste.php?critere_recherche=FchGnrl_Prenom&amp;cle=<?php echo stripslashes($prenom) ?>&amp;envoyer=Chercher&amp;exact=1" target="_top"><?php echo stripslashes($prenom) ?></a>
      </h1>
...
*/
$sql4=$pdo->prepare('SELECT * FROM fchpat_Note WHERE fchpat_Note_PatPK=?');
$sql4->bindValue(1, $primkey, PDO::PARAM_STR);
$sql4->execute();
$ligne4=$sql4->fetch(PDO::FETCH_ASSOC);
$sql4->closeCursor();

$sql5=$pdo->prepare('SELECT * FROM fchpat_Intervenants INNER JOIN Personnes ON fchpat_Intervenants.fchpat_Intervenants_IntervPK=Personnes.ID_PrimKey WHERE fchpat_Intervenants_PatGUID=?');
$sql5->bindValue(1, $patient, PDO::PARAM_STR);
$sql5->execute();
$ligne5_all=$sql5->fetchAll(PDO::FETCH_ASSOC);
$sql5->closeCursor();
...
<!-- table generale -->
	...
	<tr>
	  <td onmouseover="showHidePannel('coordinates'); return false">
<?php
if ($naissance=='invalid')
  echo 'Date de naissance non valide<br />';
else
  echo $ne." le ".$naissance;
//calcul de l'age
...
} //fin de date naissance valide
$lignes_adresse=array();
if ($ligne2["FchPat_Adresse"])
...
?>
	...
	<ul>
        ...
        <li><a  href="tableaux.php?affichage=terrains&GUID=<?php echo $patient ?>" target="droit_bas"><span>Terrains</span></a></li>
...
//echo "
//		  ".stripslashes($ligne2["FchPat_Titre"])." ".stripslashes($nom)." ".stripslashes($ligne["FchGnrl_Prenom"])."<br />";
...
//on met la premiere ligne d'adresse en qrcode
  ...
  $qrcode_string.='ADR;WORK;CHARSET=UTF-8:;';
if (count($lignes_adresse))
{
 $qrcode_string.= $lignes_adresse[0];
}
$qrcode_string.= ';'.$ligne2["FchPat_Ville"].';;'.$ligne2["FchPat_CP"]."\n";
...
<div id="create">
		  <form action="consultation.php" method="get" target="droit_bas">
			...
			Création de document
		      ...
		      <input type="checkbox" name="check_ald" id="check_ald" value="ald_on" /><label for="check_ald">ALD</label><br />
...
?>
		      ...
		      <input name="nouveau" type="submit" value="Ajouter" onsubmit="verif_ordo('<?php echo $prenom?>', '<?php echo $nom ?>', '<?php echo $naissance_full ?>')"/>
		    </fieldset>
		  </form>
...
/*	$sql_droits_delegues="SELECT FriendUserDroits FROM user_perms WHERE FriendUserGUID='$GUID_user' AND SignataireGUID='$GUID_signuser'";
	...
	{
	  if ((stripos($ligne_chercher_droits['Droits'],$elts_rubrique[1].'c') OR (stripos($droits_delegues,$elts_rubrique[1].'c'))) AND $elts_rubrique[2] !='20020100') //Ne pas proposer d'enregistrer si l'utilisateur n'a pas les droits directs ou delegues ou si ordonnance structuree
	  {
?>
		      <option value="<?php echo $elts_rubrique[2] ?>" <?php if ($elts_rubrique[2] == '20080000') echo "selected=\"selected\"" ?> >
...
Taille du cadre d'image<br />
100 %<input type='radio' name='radio_ratio' value='1'>
50 %<input type='radio' name='radio_ratio' value='2' checked="checked">
33 %<input type='radio' name='radio_ratio' value='3'>
25 %<input type='radio' name='radio_ratio' value='4'>
</select>
			<input name="image" type="submit" value="Ajouter" title="Importer un fichier image" />
			<input name="ID_PrimKey" type="hidden" value="<?php echo $primkey ?>"  />
		      </div>
		    </fieldset>
		  </form>
</div>
...
if (isset ($rubrique_perso))
{
  ...
  foreach ($rubrique_perso AS $this_rubrique_perso)
  {
    ...
    {
?>
			<input name="type_info[]" id="x<?php echo $tableau_rubrique_perso[2]; ?>" type="checkbox" value="<?php echo $tableau_rubrique_perso[2]; ?>"  />
...
<?php
if (count($ligne5_all) > 0) //affichage des intervenants
{
  echo "
	     <table>
<!-- Tableau de 3eme niveau intervenants-->";
//  while ($ligne5=mysqli_fetch_array($resultat5))
  foreach ($ligne5_all AS $ligne5)
  {
?>
		    <tr>
		      <td>
			...
			</a> <?php echo stripslashes($ligne5["Prenom"]).", ".stripslashes($ligne5["Qualite"])." ".$ligne5["fchpat_Intervenants_Type"] ?>
		      </td>
		      <td>
