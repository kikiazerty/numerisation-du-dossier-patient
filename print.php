...
//]]>
  ...
  <script type="text/javascript">
//<![CDATA[
function check_day(dayDocument) //pour alerter si document date d'un autre jour que le jour courant et imprimer si OK
{
  var now = new Date();
  ...
  var thisDay = annee+'-'+mois+'-'+jour;
  if (thisDay!=dayDocument) 
  {
    var r= confirm ("Voulez-vous vraiment imprimer ? (le document n'est pas à la date du jour)");
    if (r==true) //fenetre d'impression seulement si OK dans la boite de dialogue
    {
      window.print();
    }
  }
  else //pas de boite de dialogue si meme jour
  {
    window.print();
  }
}
...
/*
$sql="SELECT * FROM Personnes WHERE Login ='$signuser'";
$resultat=mysqli_query($db,$sql);
$ligne=mysqli_fetch_array($resultat);
*/
$sql=$pdo->prepare("SELECT * FROM Personnes WHERE Login =?");
$sql->bindValue(1, $signuser, PDO::PARAM_STR);
$sql->execute();
...
/*  $sql_document="SELECT * FROM RubriquesHead INNER JOIN RubriquesBlobs ON RubriquesBlobs.RbDate_PrimKey=RubriquesHead.RbDate_RefBlobs_PrimKey WHERE RbDate_RefBlobs_PrimKey='$id_document'";
  ...
  */
  $sql_document=$pdo->prepare("SELECT * FROM RubriquesHead INNER JOIN RubriquesBlobs ON RubriquesBlobs.RbDate_PrimKey=RubriquesHead.RbDate_RefBlobs_PrimKey WHERE RbDate_RefBlobs_PrimKey=?");
  $sql_document->bindValue(1, $id_document, PDO::PARAM_STR);
  $sql_document->execute();
  $ligne_document=$sql_document->fetch(PDO::FETCH_ASSOC);
  $sql_document->closeCursor();
  
  $patient=$ligne_document['RbDate_IDDos'];
  $date=$ligne_document['RbDate_Date']; //format iso
  $date_seule=explode(" ",$date); //format iso
  $date_affichable= iso_to_local($date_seule[0],$date_format);
}
/*
...
*/
  $sql_destinataire=$pdo->prepare("SELECT * FROM fchpat_Intervenants INNER JOIN Personnes ON fchpat_Intervenants.fchpat_Intervenants_IntervPK=Personnes.ID_PrimKey WHERE fchpat_Intervenants_PatGUID=?");
  $sql_destinataire->bindValue(1, $patient, PDO::PARAM_STR);
  $sql_destinataire->execute();
  $ligne_destinataire_all=$sql_destinataire->fetchAll(PDO::FETCH_ASSOC);
  $sql_destinataire->closeCursor();
...
*/
  $sql_patient=$pdo->prepare("SELECT * FROM fchpat WHERE FchPat_GUID_Doss=?");
...
?>
    <div class='noPrint' style="position:absolute; left:180mm; top:15mm;z-index: 100;" >
      <fieldset>
	<legend>
	  Destinataire
	</legend>
...
<?php
$i=0;
foreach($ligne_destinataire_all AS $ligne_destinataire)
...
?>
      ...
      <fieldset>
	<input type="button" value="Imprimer" onclick="check_day('<?php echo $date_seule[0] ?>')"/>
	<form action="compta/saisie_hono.php" method="get" target="_top">
	  <div>
	    <input name="guid_patient" type="hidden" value="<?php echo $patient ?>"/>
	    <input name="date" type="hidden" value="<?php echo $date_affichable ?>"/>
...
<?php
  ...
  $tab=array();
//  $tab[2]=""; //on initialise au cas où le document n'ait pas de <body>
  $expr="`(<body[^>]*>)(.*)(<\/body>)`si";
