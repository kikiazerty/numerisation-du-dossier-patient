...
*/
$sql_chercher_ordo=$pdo->prepare("SELECT * FROM RubriquesBlobs INNER JOIN RubriquesHead ON RubriquesBlobs.RbDate_PrimKey=RubriquesHead.RbDate_RefBlobs_PrimKey WHERE RubriquesHead.RbDate_IDDOs=? AND (RubriquesHead.RbDate_TypeRub='20020100')"); //on cherche l'ordo par son numero
$sql_chercher_ordo->bindValue(1, $patient, PDO::PARAM_STR);
$sql_chercher_ordo->execute();
$ligne_chercher_ordo_all=$sql_chercher_ordo->fetchAll(PDO::FETCH_ASSOC);
...
if (count($ligne_chercher_ordo_all)==0)
{
...
foreach ($ligne_chercher_ordo_all AS $ligne_chercher_ordo)
{
...
/*    $sql_GUID_possesseur_ordonnance="SELECT GUID FROM Personnes WHERE Login='$login_possesseur_ordonnance'";
  ...
  */  
    $sql_GUID_possesseur_ordonnance=$pdo->prepare("SELECT GUID FROM Personnes WHERE Login=?"); //on cherche l'ordo par son numero
    $sql_GUID_possesseur_ordonnance->bindValue(1, $login_possesseur_ordonnance, PDO::PARAM_STR);
    $sql_GUID_possesseur_ordonnance->execute();
    $ligne_GUID_possesseur_ordonnance=$sql_GUID_possesseur_ordonnance->fetch(PDO::FETCH_ASSOC);
    $sql_GUID_possesseur_ordonnance->closeCursor();
...
*/
    $sql_utilisateur_autorise_lire_ordonnances=$pdo->prepare("SELECT * FROM user_perms WHERE FriendUserGUID=? AND SignataireGUID=?"); //on cherche l'ordo par son numero
    $sql_utilisateur_autorise_lire_ordonnances->bindValue(1, $GUID_user, PDO::PARAM_STR);
    $sql_utilisateur_autorise_lire_ordonnances->bindValue(2, $GUID_possesseur_ordonnance, PDO::PARAM_STR);
    $sql_utilisateur_autorise_lire_ordonnances->execute();
    $ligne_utilisateur_autorise_lire_ordonnances=$sql_utilisateur_autorise_lire_ordonnances->fetch(PDO::FETCH_ASSOC);
    $sql_utilisateur_autorise_lire_ordonnances->closeCursor();

    
    if (count($ligne_utilisateur_autorise_lire_ordonnances))//S'il existe un utilisateur autorise
    {
 //     $liste_utilisateur_autorise_lire_ordonnances=mysqli_fetch_array($resultat_utilisateur_autorise_lire_ordonnances);
      if (preg_match("`orv`",$ligne_utilisateur_autorise_lire_ordonnances['FriendUserDroits']))//On verifie qu'il a les droits
	$lecture_ordoOK=1;
      else
	$lecture_ordoOK="";
    }
...
if ($toutes_cip)
{
  $sql_chercher_medoc=$pdodatasemp->prepare("SELECT f1,f3 FROM t00 WHERE f2= ? "); 
 ...
 /*   $sql_chercher_medoc="SELECT f1,f3 FROM t00 WHERE f2='$cette_cip'";
   ...
   */ 
    $sql_chercher_medoc->bindValue(1, $cette_cip, PDO::PARAM_STR);
